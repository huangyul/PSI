...
export default {
    ...
    methods: {
        funcGetTableData(supplierName, supplierId, shopCode, userType,topOrgId, activeStatus, page, pageSize) {
            ...
            var url = basis.SupplierManagement.queryOne + "?supplierId=" + localStorage.getItem("UpdateSupplierManagementSupplierId");
            this.$axios.get(url).then(res => {
                this.form = res.data;
								//console.log(res);
            }).catch(err => {
        ...
        eventSave() {
            ...
            for (var item of this.tableData) {
                if (item.SupplierId == localStorage.getItem("UpdateSupplierManagementSupplierId")) {
                    continue;
                }
                ...
                if (this.form.SupplierId == item.SupplierId) {
                    ...
                    return;
						...
						this.form.Modifier = localStorage.getItem("ms_username");
						const loading = func.backgroundLoading('Loading');
            var url = basis.SupplierManagement.update + "?shopCode=" + localStorage.getItem("shopCode") + "&userType=" + localStorage.getItem("userType")+
						"&topOrgId=" + localStorage.getItem("OrganizationId");
