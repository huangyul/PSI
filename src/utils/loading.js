...
class func {
  ...
  static DialogSearchJudge() {
    setTimeout(function () {
      //整个搜索div的宽度
      var searchBoxWidth = $(
        '.all .dialogVisibleSearch .searchBox'
      ).outerWidth()
      //右边按钮的宽度
      var operationWidth = $(
        '.all .dialogVisibleSearch .searchBox .operation'
      ).outerWidth(true)
      //左边内容的宽度
      var conditionsBoxWidth = searchBoxWidth - operationWidth
      //一整个搜索框内容的宽度
      var boxWidth = $(
        '.all .dialogVisibleSearch .searchBox .conditions .box'
      ).outerWidth(true)
      //一整个搜索框内容的显示数量
      var boxIndex = $(
        '.all .dialogVisibleSearch .searchBox .conditions .box:visible'
      ).length
      if (boxIndex * boxWidth > conditionsBoxWidth) {
      } else {
        $('.all .dialogVisibleSearch .searchBox .conditions').css(
          'flex',
          'none'
        )
      }
  ...
  static TracingPointFunc() {
    ...
    $('.all .TracingPointBody .titleBox').each(function (index, element) {
      ...
      if (index === 0) {
        rightDivUlHtml =
          rightDivUlHtml +
          "<div class='box active'><div class='point'></div><p>" +
          text +
          '</p></div>'
      } else {
        rightDivUlHtml =
          rightDivUlHtml +
          "<div class='box'><div class='point'></div><p>" +
          text +
          '</p></div>'
      }
    })
    ...
    //头部描点控制内容
    $('.all .TracingPointTop .TracingPointContainer .append .box').click(
      function () {
